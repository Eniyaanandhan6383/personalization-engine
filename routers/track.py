...
FAKE_ADS = [
    ...
    {"ad_id": "a016", "title": "Premium Kitchen Deals",          "category": "food",      "url": "https://example.com/kitchen",  "body": "Premium cookware at best prices"},
    # travel
...
def extract_scores(data: dict) -> dict:
    ...
    # Only keep known valid categories with score > 0
    return {
        k: v for k, v in scores.items()
        if k in VALID_CATEGORIES and isinstance(v, (int, float)) and v > 0
    }
...
def get_ads(
    ...
    if not profile:
        return {
            ...
            "hint":    "Use POST /track to log behavior first. Then ads will be personalized."
        }
    ...
    try:
        ...
        return {
            "ads":     [],
            "message": "Profile data is corrupted.",
            "hint":    "Track new events to rebuild your profile."
        }
    ...
    if not active_scores:
        return {
            "ads":     [],
            "message": "No valid interest scores found.",
            "hint":    f"Track events with valid categories: {', '.join(sorted(VALID_CATEGORIES))}"
        }
    ...
    # Safety — if somehow no ads match (shouldn't happen but just in case)
    if not matched_ads:
        return {
            "ads":     [],
            "message": f"No ads available for category '{top_category}' yet.",
            "hint":    "More ad categories are being added soon.",
    ...
    return {
        "ads":          matched_ads,
        "targeted_for": top_category,
        "confidence":   round(active_scores[top_category], 3),
        "all_scores":   active_scores,
        "total_categories_tracked": len(active_scores)
    }